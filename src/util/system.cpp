...
const char * const BITCOIN_CONF_FILENAME = "rwa.conf";
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Rwa
    // macOS: ~/Library/Application Support/Rwa
    // Unix-like: ~/.bitcoin
#ifdef WIN32
    // Windows
    //return GetSpecialFolderPath(CSIDL_APPDATA) / "Rwa";

    WCHAR pszExePath[MAX_PATH];
    GetModuleFileNameW(NULL, pszExePath, ARRAYSIZE(pszExePath));
    PathRemoveFileSpec(pszExePath);
    wcscat(pszExePath, L"\\data");
    return pszExePath;
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Rwa";
#else
    // Unix-like
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Rwa Core copyright is not removed by accident
    if (copyright_devs.find("Rwa Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Rwa Core developers";
    }
